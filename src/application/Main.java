...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		...
		buildSound = new AudioClip((ClassLoader.getSystemResource("buildsound.wav").toString()));
		buildSound.setVolume(1.0);

		moveSound = new AudioClip((ClassLoader.getSystemResource("movesound.mp3").toString()));
		...
		mp.getHelpBtn().setOnAction(e -> {
			/*
			 * Canvas canvas = new Canvas(1000,600); GraphicsContext gc =
			 * canvas.getGraphicsContext2D(); des.getChildren().add(canvas);
			 */
	...
	/*
	 * public void setBackground(GraphicsContext gc) { gc.setFill(Color.LIGHTCYAN);
	 * gc.fillRect(0, 0, gc.getCanvas().getWidth(), gc.getCanvas().getHeight()); }
	 * public void setHead(GraphicsContext gc) { Font f1 =
	 * Font.font("Serif",FontWeight.BLACK,40); gc.setFont(f1);
	 * gc.fillText("Description", 30, 60); } public void
	 * setDescription(GraphicsContext gc) { Font f2 =
	 * Font.font("Serif",FontWeight.BLACK,30); gc.setFont(f2);
